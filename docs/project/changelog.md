...
### Changed
- Template output defaults to `/tmp/spark_fuse/orders_enriched_ct` and prefers `write.change_tracking` on Delta targets, with a Parquet fallback when Delta is unavailable.
