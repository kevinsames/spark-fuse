...
def _create_long_accumulator(sc, name: str):
    ...
    _LOGGER.debug("SparkContext.longAccumulator missing; using legacy accumulator for %s", name)
    ...
    *,
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            _LOGGER.exception("Exception during LLM API call (attempt %s)", attempt)
            time.sleep(min(2**attempt, 60))
            continue

        if response.status_code == 429:
            _LOGGER.warning("Rate limit hit (HTTP 429). Backing off (attempt %s).", attempt)
            time.sleep(min(2**attempt, 60))
            continue
        ...
        try:
            ...
            _LOGGER.exception("Failed to parse LLM response as JSON")
            return None
    ...
    _LOGGER.error("LLM mapping failed after %s attempts", max_retries)
    return None
